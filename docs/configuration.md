...
Every home loses non-forced scout cleanup whatever the backlog backend, because `bin/fm-teardown.sh` settles a scout's unresolved-decision completion gate through `bin/fm-decision-hold.sh`, and every command of that script needs a compatible `tasks-axi`, so the gate can neither pass nor be cleared.
