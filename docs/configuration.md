...
The partial guard is still worth keeping because it is real enforcement of the hazard it does cover, unlike a `tasks-axi` condition, which was rejected for guarding nothing at all.
The `tasks-axi` fallback covers firstmate's own backlog editing only, so declining it also leaves `bin/fm-backlog-handoff.sh` unavailable whatever the backlog backend, because that helper delegates the item move unconditionally, and it hard-blocks remote secondmate provisioning, because `bin/fm-remote-doctor.sh` counts `tasks-axi` among its required tools and `bin/fm-remote-home-seed.sh` turns that into `remote runtime preflight failed; nothing was provisioned`.
Nothing gates that declination, since gating it on `config/backlog-backend=manual` would guard nothing, and it matters only for a home with registered secondmates.
