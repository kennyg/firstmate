...
`fm-send.sh` selects a pre-Enter popup-settle for slash commands and for codex `$...` skill invocations using metadata-routed target `harness=` values, then waits after successful text sends so immediate peeks catch the receiving turn starting, bounded by `FM_SEND_SETTLE` as a cap rather than a fixed duration: a metadata-bearing target is polled against the semantic busy-state contract and the wait ends the moment it reads busy, while a target that never reads busy waits the full cap; the sub-supervisor uses only the shared submit core and does not pay that post-submit wait.
