...
- `OPTIONAL_TOOLS: invalid config/optional-tools - <reason>` - the declined-optional-tools file names something that cannot be declined, either a tool the resolved backend requires or a name outside the declinable set (most often a typo); that name is never honored, so the tool still reports missing when it is absent.
